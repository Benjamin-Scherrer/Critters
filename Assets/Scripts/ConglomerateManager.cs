...
public class ConglomerateManager : MonoBehaviour
{
    ...
    //PRIVATE
    private void OnJointSnapped(Geobody geobody)
    {
    ...
    private void GoThroughConglomerateHierarchyStart()
    {
        ...
        geobody.SetToMainHead(mainHeadMainMovmementMult.Evaluate(Geobodies.Count), mainHeadFloatingMult.Evaluate(Geobodies.Count));
    ...
    private void GoThroughConglomerateHierarchyStep(int chainSegmentIndex, int chainDepth, Geobody geobody, List<Geobody> geobodiesAlreadyReached)
    {
        ...
        else
        {
            chainSegmentIndex = 0;
            chainDepth++;
            geobody.SetToSideHead(sideHeadMainMovementMult.Evaluate(chainDepth), sideHeadFloatingMult.Evaluate(chainDepth));
        }
